...
namespace frictionModels
{
    ...
    addToRunTimeSelectionTable(frictionModel, Quadratic, dictionary);
}
}
...
Foam::frictionModels::Quadratic::Quadratic
(
...
void Foam::frictionModels::Quadratic::resetTy() const
{
    Ty_ = dimensionedScalar(dimPressure);
}

void Foam::frictionModels::Quadratic::reseteta() const
{
    eta_ = dimensionedScalar(dimDynamicViscosity);
}

void Foam::frictionModels::Quadratic::calcTy() const
{
    resetTy();
...
void Foam::frictionModels::Quadratic::calceta() const
{
    reseteta();
...
const Foam::areaVectorField& Foam::frictionModels::Quadratic::tauSc() const
{
    resetTauSc();
...
const Foam::areaScalarField& Foam::frictionModels::Quadratic::tauSp() const
{
    resetTauSp();
    calcTy();
    calceta();
    ...
    // Viscous stress
    tauSp_ += 1./(8*(h_+h0_)) * K_ * eta_;

    // Turbulent-dispersive stress
    tauSp_ += rho_*n_*n_*g_.value()*u/pow((h_+h0_)*dimensionedScalar("n2", dimAntiL, 1.0), 1/3.);

    return tauSp_;
}


bool Foam::frictionModels::Quadratic::read
(
    const dictionary& frictionProperties
)
{
...
const Foam::scalar& Foam::frictionModels::Quadratic::ut
(
    const float flux, 
...
const Foam::scalar& Foam::frictionModels::Quadratic::htu
(
    const float flux, 
    const List<scalar> wide,
    const List<scalar> slope,
...
const Foam::List<Foam::scalar>& Foam::frictionModels::Quadratic::ht
(
    const float flux, 
    const List<scalar> wide,
    const List<scalar> slope,
